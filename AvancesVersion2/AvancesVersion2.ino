...
void seleccionarJugador(void) {
  ...
  Rect(114, 50, 46, 50, 0x301F);
  Rect(116, 51, 45, 49, 0x301F);


  Rect(160, 50, 46, 50, 0xE800);
  Rect(161, 51, 45, 49, 0xE800);

      struct control control1 = {analogRead(PE_3), analogRead(PE_0), digitalRead(PF_1)};
    struct control control2 = {analogRead(A9), analogRead(A8),  digitalRead(PA_7)};

  while (jugadoresListos != 1) {

    Serial.print(control2.ataque1);
     Serial.println(control1.ataque1);
  ...
  if (jugador1Listo == 1 && jugador2Listo == 1) {
    jugadoresListos = 1;
...
//Primerjugador
   //Izquierda
  if (control1.horizontal <= 1000) {
  }

  //Derecha
   if (control1.horizontal >= 3000) {
  }
...
//SegundoJugador
  //Derecha
  if (control2.horizontal <= 1000) {
  }

  //Izquierda
    if (control2.horizontal >= 3000) {
  }
